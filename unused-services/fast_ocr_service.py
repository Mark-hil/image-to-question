"""
Fast OCR Service using PaddleOCR without OpenCV dependencies.
This provides much faster OCR than Tesseract.
"""
import os
import numpy as np
...
class FastOCRService:
    """Fast OCR service using PaddleOCR for minimal dependencies."""
    
    def __init__(self, lang: str = 'en'):
        """
        Initialize fast OCR service.
    ...
    def initialize(self):
        ...
        if self.ocr_engine is None:
            ...
            try:
                # Minimal PaddleOCR initialization for speed
    ...
    async def extract_text_from_image(self, image_path: str) -> Dict[str, str]:
        """
        Extract text from image using fast PaddleOCR.
        """
        self.initialize()
        
        try:
            # Read image with PIL (no OpenCV needed)
    ...
    def _run_ocr(self, image_array: np.ndarray) -> List:
        """Run PaddleOCR on image array."""
        return self.ocr_engine.ocr(image_array)
...
# Global fast OCR service instance
_fast_ocr_service = FastOCRService(lang='en')

async def extract_text_from_path(file_path: str) -> Dict[str, str]:
    """
    Fast text extraction using PaddleOCR service.
    """
    file_ext = os.path.splitext(file_path)[1].lower()
    
    if file_ext == '.pdf':
        return await _fast_ocr_service.extract_text_from_pdf(file_path)
    else:
        return await _fast_ocr_service.extract_text_from_image(file_path)
